...
def implement_transfer_function(character_indicator, tz,  k, alpha, beta, time):
        ...
        for j in range(n_mv):
            if dependency[i][j] == 1:
                ...
                for p in range(0, mass_n[i, j]):
                    w.append(implement_transfer_function(character_indicator[i][j], tz[i][j], k[i][j], alpha[i][j], beta[i][j],\
                    delta_T*(p+1)))
                s_per.append(w)
        s.append(s_per)
